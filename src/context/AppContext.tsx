...
interface AppContextType {
  ...
  updateCartQuantity: (productId: string, lensType: string, quantity: number) => void;
  clearCart: () => void;
  getCartCount: () => number;
  getCartTotal: () => number;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [wishlist, setWishlist] = useLocalStorage<WishlistItem[]>('optivue-wishlist', []);

  const addToCart = useCallback((product: Product, lensType: string, quantity = 1) => {
  ...
  const removeFromCart = useCallback((productId: string, lensType: string) => {
    setCart((prev: CartItem[]) =>
      prev.filter((item) => !(item.product.id === productId && item.lensType === lensType))
    );
  }, [setCart]);

  const updateCartQuantity = useCallback((productId: string, lensType: string, quantity: number) => {
    if (quantity <= 0) {
  ...
  const placeOrder = useCallback((customer: Order['customer']) => {
    const orderId = `OV-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).substr(2, 4).toUpperCase()}`;
    const order: Order = {
      id: orderId,
